...
def cst_metric_xgb(pred, dtrain):
    label = logistic(dtrain.get_label())
    pred = logistic(pred)
...
def train():
    ...
    y_train = logit(df['t_deal_probability'].values)
    ...
    for params in tqdm(list(ParameterGrid(all_params))):
        ...
        for train, test in cv.split(x_train, y_train):
            ...
            pred = logistic(clf.predict(val_x)).clip(0, 1)
...
def predict():
    ...
    p_test = logistic(clf.predict(x_test))
