...
def proc(img_id):
    ...
    for i, line in enumerate(open(f'../data/img_info/{img_id}.jpg.txt', 'r')):
        ...
        col, val = line
        if col == {'004_Geometry', '063_Pagegeometry'}:
            ...
            df_new[col] = pd.to_datetime(val[7:]).astype(int) // 10**9
        ...
        elif col in {'015_Pixels', '021_kurtosis', '022_skewness', '023_entropy', '029_kurtosis', '030_skewness', '031_entropy', '037_kurtosis', '038_skewness', '039_entropy', '046_kurtosis', '047_skewness', '048_entropy', '067_Quality', ''}:
            df_new[col] = float(val)
    return df_new


# with Pool() as p:
#    #rets = list(p.map(proc, tqdm(ids), chunksize=10000))
